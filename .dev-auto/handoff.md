...
- Carried: no Codex runtime measurement (three attempts, all dead at the trust step), `hooks: null` reading `unusable`, sampled differential padding, and Codex's cheap timeout/matcher rules — wrong either way: too strict costs a duplicate, too loose suppresses an install. Claude `timeout: 0` is measured dead, so no longer carried.
