...
- Named follow-up, from a post-acceptance `gpt-6-astra/medium` discussion: **separate Codex configuration detection from runtime verification** — qualify Codex's `wired` as "entry present; runtime unverified" and apply that uniformly, including to BMN's own entries. He explicitly said it neither blocks the push nor reopens Epic 15, and refuted the alternative of rejecting every numeric `timeout`: BMN installs `timeout: 5` itself (`bin/bmn:661`), so that rule would call BMN's own entry missing forever.
